...
export function DynamicSummary({category, current}) {
    ...
    return <Steps progressDot direction="vertical">
        {getCategoryItems(category).map(({title, description}, index) => <Steps.Step title={title} description={description} status={getStatus(index, current)}/>)}
    </Steps>
}