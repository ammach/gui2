...
export function FooterWithNavigation({previous, next}) {
    const {category: previousCategory, details: previousDetails = '', path: previousPath} = previous;
    const {category: nextCategory, details: nextDetails = '', path: nextPath} = next;
    ...
    return <Footer>
        <div className="footer-buttons-container">
            ...
            <div className="footer-buttons-right">
                <Button text="suite" icon={() => <ArrowRightOutlined/>} onClick={handleNextClick}/>
            </div>
        </div>
